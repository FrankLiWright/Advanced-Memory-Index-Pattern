...
| 信息类型 | 归属 | 示例 |
|----------|------|------|
| 用户下载偏好 | MEMORY | 离线部署包、国内镜像优先 |
| 用户输出风格 | MEMORY | 简洁、文学风格、技术写作规范 |
| 用户身份 | USER PROFILE | 姓名、职业、兴趣 |
| 安装路径、端口 | 外部文档 | 程序路径、服务端口 |
| 工具启用/禁用状态 | 外部文档 | 某工具已禁用、替代方案 |
| 活跃项目状态 | 外部文档 | 当前版本、进度追踪 |
| 工作流详情 | 外部文档 | 处理策略、流程步骤 |
...
```
对话记忆：<文档位置>，含<内容摘要> [→ <文档标识>]
```

示例：
```
对话记忆：Craft「工作记录/对话记忆」，含活跃项目+系统配置+文件索引+对话历史+用户规则 [→ doc:abc123]
```
...
### Q: 外部文档太多怎么办？
**A:** 保持 2-3 个文档为佳。一个「对话记忆」文档用章节区分即可，无需拆分过多文件。
