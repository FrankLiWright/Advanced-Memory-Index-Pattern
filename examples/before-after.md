...
### Before (in USER PROFILE, ~200 chars)
```
用户全名李鋆灏（Frank Li Wright），南京市外国语学校高二（9）班学生。
文艺青年+理工思维：音乐剧《回声》、傅里叶级数可视化、模联经历。
写作风格：善用比喻、有哲理、古今融汇。Token节约：输出简洁，非文学
场景去冗余。
```

### After (USER PROFILE ~120 chars)
**USER PROFILE:**
```
用户全名李鋆灏（Frank Li Wright），高中生。文艺青年+理工思维。
写作风格：善用比喻、有哲理、古今融汇。Token节约：输出简洁，非文学场景去冗余。
```

**Changes:**
- Removed specific class ("高二（9）班") — changes annually, generalized to "高中生"
- Removed specific projects (music show, Fourier visualization, MUN) — move to session history
- Kept: name, identity archetype, writing style, token preference

**Savings: ~80 chars (40%)**

---

## Example 4: MEMORY Pointer Pattern
